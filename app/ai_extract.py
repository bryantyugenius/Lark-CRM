...
- next_action_at 格式为 YYYY-MM-DD，基于今天的日期计算。例如"下周一"、"明天"等
- 如果无法判断具体日期则填 null，不要随意猜测日期
...
async def extract_structured(text: str) -> ExtractedInteraction:
    ...
    prompt = EXTRACT_PROMPT.format(today=today.isoformat(), weekday=weekday)
    ...
    try:
        ...
        # 日期校验：AI 可能幻觉，用规则引擎修正
        if result.next_action_at:
            ai_date = result.next_action_at
            rule_date = _parse_date(text)
            # 如果 AI 日期偏离今天超过 2 年，明显是幻觉
            try:
                parsed = date.fromisoformat(ai_date)
                delta_days = abs((parsed - today).days)
                if delta_days > 730:  # 超过 2 年 = 幻觉
                    print(f"  ⚠️ AI date hallucination: {ai_date}, rule suggests: {rule_date}")
                    result.next_action_at = rule_date
                # 如果规则引擎有结果且与 AI 不同，优先规则引擎
                elif rule_date and rule_date != ai_date:
                    print(f"  ⚠️ AI date {ai_date} differs from rule {rule_date}, using rule")
                    result.next_action_at = rule_date
            except ValueError:
                print(f"  ⚠️ Invalid AI date: {ai_date}, using rule: {rule_date}")
                result.next_action_at = rule_date
